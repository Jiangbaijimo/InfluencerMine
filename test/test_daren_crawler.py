...
def fetch_daren_data() -> Dict[str, Any]:
    """
    模拟 cURL 请求，从巨量星图获取达人数据。
    ...
    headers = {
        ...
        # 直接使用 1.txt 中 -b 参数后的完整 Cookie 字符串
...
def main():
    ...
    if raw_data.get('base_resp', {}).get('status_code') != 0:
        print(f"API 返回错误: {raw_data.get('base_resp', {}).get('status_message')}")
        return
    ...
    print(f"成功获取并解析了 {len(parsed_authors)} 位达人数据。")

    # 4. 保存到 CSV
    save_to_csv(parsed_authors)
